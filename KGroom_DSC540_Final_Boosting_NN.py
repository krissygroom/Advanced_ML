...
if binning==0 and cross_val==0:
    ...
    rgr=GradientBoostingRegressor(n_estimators=100, loss='ls', learning_rate=0.1, max_depth=3, min_samples_split=3, random_state=None)
    rgr.fit(data_train, target_train)
    print('Gradient Boosting RMSE:', math.sqrt(metrics.mean_squared_error(target_test, rgr.predict(data_test))))
    print('Gradient Boosting Expl Var:', metrics.explained_variance_score(target_test, rgr.predict(data_test)))
...
'''
    ...
    #SciKit Ada Boosting - Cross Val
    start_ts=time.time()
    rgr=AdaBoostRegressor(n_estimators=100, base_estimator=None, loss='linear', learning_rate=0.5, random_state=None) 
    scores=cross_validate(estimator=rgr, X=data_np, y=target_np, scoring = scorers, cv=5)                                                                                                   
